...
\noindent\textsc{Driving}: National Car Park at Greenwich with public parking available. Walking distance ($\sim$10 minutes) from Greenwich town center along the Thames.

\noindent\textsc{From Major Airports}: Gatwick (45 min via rail + tube), Stansted (60 min), Luton (70 min).
...
\textsc{Flamsteed House} (1675, Sir Christopher Wren, architect):
\begin{itemize}
...
\textsc{Meridian Building} (1884, reconstructed 1950s):
\begin{itemize}
...
\textsc{Astronomy and Time Collection}:
\begin{itemize}
...
\noindent\textsc{Admission}: Generally free to museum galleries; small charge for special exhibitions and planetarium shows. Combined pass available for Observatory + Museum.

\section{Astronomy Centre and Educational Programs}

The museum hosts ongoing educational initiatives for students and researchers:

\textsc{Courses and Workshops}:
\begin{itemize}
...
\textsc{Recommended Exhibits}:
\begin{enumerate}
\item \textsc{Harrison Chronometers} (Maritime Museum): Study the mechanical solutions to longitude determination. The display includes working models demonstrating the bimetallic strip temperature compensation and grasshopper escapement.
\item \textsc{Airy Transit Circle Replica} (Observatory): The working replica demonstrates the optical principles of meridian instruments. The micrometer screw (linear motion translated to angular rotation) shows precision engineering of the 19th century.
\item \textsc{Photographic Zenith Tube Display} (Museum archive section): Learn how 20th-century observatories automated position measurement via photographic plate analysis.
\item \textsc{Earth Orientation Exhibits} (Observatory): Displays on precession, nutation, and Chandler wobble with interactive models showing 18.6-year nutation cycle and 41,000-year precession cycle.
\item \textsc{Time Standards Timeline} (Museum): Visual representation of transition from solar time → mean solar time → sidereal time → atomic time, with equations and historical context.
\end{enumerate}

\textsc{Suggested Mathematical Deep-Dives}:
\begin{itemize}
...
\section{Beyond Greenwich: Related Sites and Resources}

\textsc{Nearby Institutions}:
\begin{itemize}
\item \textsc{Queen Mary, University of London} (5 miles): Department of Astronomy; seminars and colloquia open to public
\item \textsc{University of Cambridge, Institute of Astronomy} (50 miles): Summer school programs in astronomical history; archive containing copies of Bradley-era observation records
\item \textsc{Oxford University, History of Science Museum} (60 miles): Instruments from Oxford's medieval astronomy programs; exhibits on Halley and Bradley's Oxford connections
\end{itemize}

\textsc{Digital Resources}:
\begin{itemize}
\item \textsc{International Astronomical Union (IAU)}: Technical documentation on celestial coordinate systems, precession/nutation models, Earth orientation parameters
\item \textsc{SOFA (Standards of Fundamental Astronomy) Library}: Free software implementing SOFA C/FORTRAN algorithms for astronomical calculations (transformations, time scales, etc.); includes historical models
\item \textsc{NASA JPL Horizons System}: Ephemerides and historical position data; allows verification of Greenwich Observatory measurements against modern ephemerides
\item \textsc{Greenwich Observatory Archives (online)}: Digitized records, photographic plates, and astronomical observations (1675--1970s) available for research
\item \textsc{Bibliography of Greenwich Observatory}: Curated collection of publications, with links to full text where available
\end{itemize}

\textsc{Recommended Reading}:
\begin{itemize}
...
\begin{tabular}{llc}
\hline
\textsc{Date} & \textsc{Event} & \textsc{Equation of Time} \\
\hline
