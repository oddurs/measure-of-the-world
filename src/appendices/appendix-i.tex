...
\section{Unit Conversions: Historical and Modern Astronomical Units}

\subsection{Linear Units}

Astronomical observations from the 17th--19th centuries frequently used historical measurement units. Modern computations require conversion to metric (SI) units. Table \ref{tab:length-units} presents conversions between historical systems and modern SI units, organized by measurement category.
...
\begin{sidewaystable}[p]
  ...
  \begin{tabular}{lllll}
    \toprule
    \textsc{Unit} & \textsc{Abbrev.} & \textsc{Metric Equivalent} & \textsc{Notes} & \textsc{Historical Use} \\
    \midrule
...
Astronomical measurements require angular precision. Conversion between degrees, arc-minutes, arc-seconds, and radians enables comparison across epochs. Table \ref{tab:angular-units} shows the fundamental angular units and the progression of measurement accuracy from the 1700s to modern satellites.
...
\begin{sidewaystable}[p]
  ...
  \begin{tabular}{lllll}
    \toprule
    \textsc{Unit} & \textsc{Abbrev.} & \textsc{Relation} & \textsc{Decimal Degrees} & \textsc{Typical Measurement} \\
    \midrule
    ...
    Radian & rad & 1 rad $= 180^{\circ}/\pi$ & $57.2958^{\circ}$ & Mathematical calculations \\
    \midrule
    & & & &  \\
    \multicolumn{5}{c}{\textsc{Historical Accuracy Progression}} \\
    \midrule
...
Historical values of fundamental constants improved dramatically over centuries as observational precision increased. Table \ref{tab:astronomical-constants} lists key constants with historical estimates and modern values, illustrating the refinement of fundamental knowledge through improved measurement techniques.
...
\begin{sidewaystable}[p]
  ...
  \begin{tabular}{lllll}
    \toprule
    \textsc{Constant} & \textsc{Historical Value (Era)} & \textsc{Historical Error} & \textsc{Modern Value} & \textsc{Unit} \\
    \midrule
    \multicolumn{5}{c}{\textsc{Solar System}} \\
    \midrule
    ...
    \midrule
    \multicolumn{5}{c}{\textsc{Orbital Parameters}} \\
    \midrule
    Earth orbital eccentricity & 0.0167 (1700s, Bradley) & $\pm 0.0001$ & $0.0167086$ & (dimensionless) \\
    Obliquity of ecliptic & $23^{\circ} 28' 20''$ (1700s) & $\pm 10''$ & $23^{\circ} 26' 21.45''$ (J2000.0) & deg-min-sec \\
    ...
    \midrule
    \multicolumn{5}{c}{\textsc{Fundamental Physics}} \\
    \midrule
...
\section{Extended Instrument Comparison}

Table \ref{tab:extended-instruments} provides a comprehensive list of major astronomical instruments with technical specifications, operational periods, and historical context, demonstrating the evolution of observational capabilities from the 17th century to the present.
...
\begin{sidewaystable}[p]
  ...
  \begin{tabular}{lllllll}
    \toprule
    \textsc{Instrument} & \textsc{Date} & \textsc{Type} & \textsc{Aperture} & \textsc{Focal Length} & \textsc{Accuracy} & \textsc{Location} \\
    \midrule
    \multicolumn{7}{c}{\textsc{17th--18th Century Meridian Instruments}} \\
    \midrule
    ...
    \midrule
    \multicolumn{7}{c}{\textsc{19th Century Telescopes and Instruments}} \\
    \midrule
    ...
    \midrule
    \multicolumn{7}{c}{\textsc{Photographic Era Instruments (1900--1970)}} \\
    \midrule
    ...
    \midrule
    \multicolumn{7}{c}{\textsc{Modern Era Instruments (1970--Present)}} \\
    \midrule
...
\section{Sample Observation: Bradley's Aberration Measurement}
...
\begin{itemize}
\item \textsc{Star}: $\gamma$ Draconis ($\alpha = 17^{\mathrm{h}}56^{\mathrm{m}}36^{\mathrm{s}}$, $\delta = +51^{\circ}29'20''$ modern J2000.0)
\item \textsc{Observer}: James Bradley, Greenwich Observatory
\item \textsc{Instrument}: Zenith sector (80 mm aperture, $\pm 1$ arcsecond accuracy)
\item \textsc{Observation Dates}: September 1725 (star rising toward zenith), December 1725 (star receding from zenith)
\end{itemize}
...
\begin{tabular}{llll}
  \toprule
  \textsc{Observation Date} & \textsc{Observed Zenith Distance} & \textsc{Expected (no motion)} & \textsc{Deviation} \\
  \midrule
...
\section{Chronometer Rate Stability Example}
...
\begin{table}[!ht]
  ...
  \begin{tabular}{lll}
    \toprule
    \textsc{Day} & \textsc{Rate (seconds/day)} & \textsc{Deviation from Mean} \\
    \midrule
    1 & $+0.1$ & $-0.1$ \\
    2 & $+0.3$ & $+0.1$ \\
    3 & $+0.2$ & $+0.0$ \\
    ...
    \midrule
    \textsc{Mean Rate} & $+0.2$ s/day & \textsc{Standard Deviation: } $\pm 0.13$ s/day \\
    \bottomrule
  \end{tabular}
\end{table}

\noindent\textsc{Significance}: A chronometer with $\pm 0.4$ s/day rate stability introduces $\pm 0.4 \times 60 = \pm 24$ second error over 60 days at sea. Since 1 second of time corresponds to 15 arcseconds of longitude ($360^{\circ}/24$ hours per second), a 24-second error corresponds to $24 \times 15 = 360$ arcseconds $= 6$ minutes of arc, enabling determination of longitude to within $\pm 1$ minute of arc (roughly 1 nautical mile at equator) after compensation.