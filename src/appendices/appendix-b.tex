...
\section{B.1 Meridian Instruments}

Meridian instruments are transit circles, quadrants, and sectors designed to measure celestial positions along the observer's meridian (north-south line through the zenith).
...
\begin{table}[!ht]
  ...
  \begin{tabular}{llllllll}
    \toprule
    \textsc{Instrument} & \textsc{Date} & \textsc{Maker} & \textsc{Aperture} & \textsc{Circle} & \textsc{Grad.} & \textsc{Accuracy} & \textsc{Notes} \\
    \midrule
...
The progression shows increasing optical aperture (light-gathering), finer graduation intervals (angular resolution), and improved accuracy. The Airy transit circle represents the peak of mechanical precision before electronic recording.

\section{B.2 Telescopes}

Refractors and reflectors used for both positional astronomy and spectroscopy, ranging from small refractors to the largest reflectors of the 20th century.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{llllll}
    \toprule
    \textsc{Telescope} & \textsc{Aperture} & \textsc{Focal Length} & \textsc{Type} & \textsc{Maker/Era} & \textsc{Location} \\
    \midrule
...
Note: Speculum metal (copper-tin alloy) mirrors required regular re-polishing; modern aluminum and silver coatings (post-1900) eliminate this maintenance burden.

\section{B.3 Navigational Instruments}

Sextants, quadrants, and octants used by navigators to measure altitude angles for celestial navigation.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{llllll}
    \toprule
    \textsc{Type} & \textsc{Era} & \textsc{Makers} & \textsc{Arc Range} & \textsc{Reading Precision} & \textsc{Notes} \\
    \midrule
...
All reading precisions assume good atmospheric conditions and experienced observers. Actual shipboard accuracy was typically 3–5 times worse due to ship motion and rough seas.

\section{B.4 Chronometers}

Portable mechanical timepieces designed for sea trials, showing the progression from Harrison's experimental models to production marine chronometers.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{llllll}
    \toprule
    \textsc{Chronometer} & \textsc{Date} & \textsc{Escapement} & \textsc{Compensation} & \textsc{Rate/Day} & \textsc{Notes} \\
    \midrule
...
The progression from H1 to H5 shows systematic improvements. Rates (error per day) decreased from $\pm 5$ seconds to $\pm 0.1$ seconds—a 50-fold improvement enabling reliable ocean navigation.

\section{B.5 Clocks}

Fixed observat ory clocks and standard clocks used for timekeeping, spanning mechanical pendulums to atomic oscillators.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{llllll}
    \toprule
    \textsc{Clock} & \textsc{Date} & \textsc{Escapement} & \textsc{Compensation} & \textsc{Accuracy} & \textsc{Notes} \\
    \midrule
...
The improvement in accuracy is dramatic: from seconds per day (17th century) to seconds per million years (20th century)—a factor of $\sim 10^{10}$ improvement over 350 years.
