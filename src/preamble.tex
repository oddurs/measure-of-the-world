...
%
% Trim size: US Trade 6" × 9" (perfect-bound paperback, printed by Mixam)
% Stock:     6.25" × 9.25" (0.125" bleed on every edge, per Mixam spec)
% Font:      11pt Latin Modern body text on ~14.3pt leading
% Text block: 4.5" wide (~68 characters per line), 7.375" tall
% Margins:   inner 0.875" (gutter), outer 0.625", top 0.75", bottom 0.875"
%            All content sits well inside Mixam's 0.25" quiet area.
% =====================================================================
...
% Package: inputenc - Character encoding
% Specify UTF-8 encoding to handle accented characters and special symbols
\usepackage[utf8]{inputenc}

% Package: fontenc - Output font encoding
% Use T1 encoding for better font rendering and hyphenation
\usepackage[T1]{fontenc}

% Package: lmodern - Latin Modern fonts
% Provides better Unicode and accent support compared to Computer Modern
\usepackage{lmodern}

% Page layout: memoir's native stock/trim/margin system.
% (The geometry package is deliberately NOT used: it fights memoir's
% layout engine and cannot express a bleed area.)
%
% Stock is the physical PDF page; trimmed size is the finished book page.
% The 0.125" difference on each edge is the bleed Mixam trims away.
\setstocksize{9.25in}{6.25in}          % PDF page: height, width (with bleed)
\settrimmedsize{9in}{6in}{*}           % Finished page: height, width
\settrims{0.125in}{0.125in}            % Trim from top and fore-edge; centers the page

% Horizontal margins: {spine}{fore-edge}{ratio}. Spine margin is larger
% because perfect binding swallows ~0.25" of the inner margin.
\setlrmarginsandblock{0.875in}{0.625in}{*}

% Vertical margins: {upper}{lower}{ratio}
\setulmarginsandblock{0.75in}{0.875in}{*}

% Header/footer: headheight, footskip (baseline of text to baseline of folio)
% Headers are empty in this design; footskip keeps the page number 0.5"
% above the trim edge, i.e. outside the 0.25" quiet area.
\setheadfoot{\onelineskip}{0.375in}
\setheaderspaces{*}{\onelineskip}{*}

% Apply the layout. Must run after every \set...{} call above.
\checkandfixthelayout

% PDF page boxes for the printer. Units are PostScript points (1in = 72bp).
% Stock page = 450 x 666bp. TrimBox = the 6x9 finished page, inset 9bp.
% Mixam reads the TrimBox to locate the finished page inside the bleed.
\pdfpageattr{/TrimBox [9 9 441 657] /BleedBox [0 0 450 666]}

% Package: pdflscape - Landscape page orientation for large tables
% Allows individual pages to be rotated to landscape without rotating entire document
\usepackage{pdflscape}

% Package: rotating - Rotated floats (figures and tables)
% Provides sidewaystable and sidewaysfigure for content rotated 90 degrees
% Keeps page orientation portrait while content is rotated
\usepackage{rotating}
...
% memoir's 11pt baseline is 13.6pt; 1.05 x 13.6 = ~14.3pt leading (11/14.3),
% a conventional book setting for a 4.5" measure.
% This is the primary control for document density and readability
\linespread{1.05}\selectfont

% Paragraph indentation: first-line indent (~1.4em at 11pt)
...
% --------- LINE BREAKING AT A NARROW MEASURE ---------
% A 4.5 inch measure gives TeX far fewer break points than the US Letter
% width this text was written against, and a paragraph carrying a long
% technical word ("transformations", "chronometrical") can fail to break at
% all, setting a line past the margin.
%
% \emergencystretch gives TeX one extra pass with additional glue, used only
% when a paragraph would otherwise overflow. It costs nothing on paragraphs
% that already fit. This is the standard remedy and it removes most overfull
% lines in narrow settings without loosening the rest of the page.
\setlength{\emergencystretch}{2.2em}

% Slightly relax the badness TeX will accept before it complains. The default
% of 200 is strict for a measure this narrow.
\tolerance=800

% Discourage, but do not forbid, consecutive hyphenated lines.
\doublehyphendemerits=10000
\finalhyphendemerits=10000
% Never hyphenate the last word of a paragraph onto a line of its own.
\widowpenalty=10000
\clubpenalty=10000
...
% Angular-measure marks used inline in math: 5.3\arcsec, 2\arcmin
...
\usepackage{graphicx}
\graphicspath{{figures/}{figures/generated/}{figures/photos/}{figures/jpg/}{figures/png/}{figures/pdf/}}
...
% Package: tabularx - Tables with automatic column width adjustment
% Provides \begin{tabularx} environment with X column type for flexible widths

% Package: array - Enhanced column formatting for tables
% Provides >{\cmd} and <{\cmd} syntax for column pre/post processing

% Package: etoolbox - Hooks for environments and commands
% Provides \AtBeginEnvironment and \AtEndEnvironment for global setup
...
% Package: xcolor - Color management and definitions
% Allows definition of custom colors and colored text
\usepackage{xcolor}

\usepackage[inline]{enumitem}
\usepackage{booktabs,longtable,tabularx,array,etoolbox,caption}

% tabularx's X column justifies its text. In a column only a couple of inches
% wide that opens rivers of white space and forces bad hyphenation, so make
% every X ragged-right. Redefining \tabularxcolumn is the supported way to do
% this: X itself must keep its meaning for the width algorithm to work.
% \arraybackslash restores \\ as the row separator, which \raggedright takes.
\renewcommand{\tabularxcolumn}[1]{>{\raggedright\arraybackslash}p{#1}}

% Apply table text size globally to all table environments
\AtBeginEnvironment{table}{\tabletextsize}
...
% - acronym: Creates separate acronym glossary type
% - nonumberlist: Don't show page numbers in glossary
% - nogroupskip: No extra space between letter groups
% - nopostdot: No period after descriptions
% - section: Use section level (not chapter) for glossary headings
% Requires \makeglossaries call in document and glossary term definitions
\usepackage[acronym,nonumberlist,nogroupskip,nopostdot,section=section]{glossaries}

% Use list style (inline, no page breaks)
\setglossarystyle{list}

% Disable automatic page breaks before glossaries
\renewcommand{\glsclearpage}{}

% Remove the default glossary section header (we provide our own)
...
% Package: imakeidx - Enhanced index generation with two-column layout
\usepackage{imakeidx}
\makeindex[intoc, columns=2, title=Index]

% Index formatting: use chapter-level heading, no clear page before
...
% =====================================================================
% Typographic scale: roughly 1.2x steps from the 11pt body
% This creates visual hierarchy through size while maintaining harmony
%
% Hierarchy:
%   Chapter:        17pt (small caps, dominant)
%   Section:        13pt (small caps, primary)
%   Subsection:     11pt (italic, secondary)
%   Subsubsection:  11pt (italic, tertiary)
%   Body text:      11pt (roman)

% CHAPTER HEADING
% \chapnamefont controls "Chapter X" label (13pt italic dark gray, like H2)
% \chapnumfont controls the chapter number (13pt italic dark gray)
% \chaptitlefont controls "Chapter Title" text (small caps, dominant size, black)
\renewcommand{\chapnamefont}{\normalfont\itshape\fontsize{13}{15.6}\selectfont\color[HTML]{333333}}
\renewcommand{\chapnumfont}{\normalfont\itshape\fontsize{13}{15.6}\selectfont\color[HTML]{333333}}
\renewcommand{\chaptitlefont}{\normalfont\scshape\fontsize{17}{20.4}\selectfont\color{black}}
...
% SECTION HEADING
% 13pt small caps - primary hierarchical level below chapter
% \setsecheadstyle sets font and sizing for \section{}
% Ragged right: a long heading in small caps has few break points and will
% otherwise set past the margin at a 4.5 inch measure.
\setsecheadstyle{\normalfont\scshape\fontsize{13}{15.6}\selectfont\raggedright}
...
% SUBSECTION HEADING
% 11pt italic - secondary hierarchical level (same size as body)
...
% 11pt italic - tertiary level, minimal visual emphasis
...
% Define custom color for list decorations
\definecolor{listdecoration}{HTML}{666666}
...
\setlist[itemize]{%
  ...
  parsep=0pt,
  leftmargin=1em,
  label=\color{listdecoration}\scalebox{0.75}{\textbullet}%
}
...
% --------- TABLE TEXT SIZING ---------
% Body text size for table content (9pt for compact tables)
\newcommand{\tabletextsize}{\fontsize{9}{11}\selectfont}
...
% Captions are set in small font (slightly smaller than body text)
% with proportional line height matching body text's 1.25× multiplier
% Body: 11pt with ~14.3pt leading → Caption: 9pt with 11.5pt leading
\newcommand{\captionformat}{\fontsize{9}{11.5}\selectfont}
\captionsetup{font={small},labelfont=small,justification=centering}
\DeclareCaptionFormat{default}{\captionformat #1#2#3}

% --------- TABLE NOTES STYLING ---------
% Table notes appear below the table in the same style as captions
% Use \tablenote{} for single notes or \begin{tablenotes} for multiple
\newcommand{\tablenote}[1]{%
  \par\vspace{6pt}%
  {\captionformat #1}%
}

% Multiple table notes environment (like footnotes within tables)
...
% --------- SMALL CAPS CUSTOMIZATION ---------
% Redefine \textsc to use slightly smaller size and neutral letter spacing
% Small caps are 0.9× base size with no extra tracking (letter spacing)
% Prevents excessive size and ensures consistent spacing within small caps
\renewcommand{\textsc}[1]{{\fontsize{10}{12.5}\selectfont\scshape\textls*[0]{#1}}}
...
% --------- HEADER TEXT STYLING ---------
% 10pt italic for headers (slightly smaller than body text)
% Used with \versoheader{} and \rectoheader{} commands
\newcommand{\headerfont}{\normalfont\itshape\fontsize{10}{12}\selectfont}

% --------- FOOTER TEXT STYLING ---------
% 9pt for footer content (minimal information, needs small size)
\newcommand{\footerfont}{\normalfont\fontsize{9}{11}\selectfont}

% --------- PAGE NUMBER STYLING ---------
% 10pt for page numbers (same size as headers for consistency)
...
\newcommand{\headerindent}{0.1875in}          % Indent for header content
\newcommand{\headermargin}{0.1875in}        % Margin for header rules
\newcommand{\footerindent}{0.1875in}          % Indent for footer content
...
\newcommand{\runningheadskip}{0\baselineskip}
...
% Maximum width of margin note text. The 6x9 outer margin is only 0.625",
% so margin notes are effectively unavailable in the print layout.
\newcommand{\marginnotewidth}{0.4in}

% Horizontal gap between body text and margin note
\newcommand{\marginnotesep}{0.125in}
...
\newcommand{\skippagefootins}{0.5\baselineskip}   % Gap before first footnote
...
\renewcommand{\footnotesize}{\fontsize{9}{11.5}\selectfont}

% --------- FOOTNOTE MARKER SPACING ---------
% Space after superscript footnote number before content
% Note: Memoir's footnote spacing is controlled by the memoir class internals
% To adjust, would need post-document processing or custom document hooks
...
% Distance from body text baseline to the page-number baseline is set by
% \setheadfoot{}{} in Section 1 (currently 0.375in). Adjust it there.
% =====================================================================
...
% - Line spacing:         \linespread value (Section 3)
% - Trim size / margins:  memoir layout commands (Section 1)
