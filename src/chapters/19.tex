...
So rotating the mirror by $\beta$ changes the outgoing ray angle by $2\beta$. For the navigator, this means that a $1^{\circ}$ rotation of the index mirror produces a $2^{\circ}$ change in the angle between horizon and star as viewed through the telescope. An observer reading an arc directly graduated can measure angles up to twice the arc's full span. A $60^{\circ}$ arc measures angles up to $120^{\circ}$—sufficient to encompass the angular separation between Sun and Moon at favorable lunar distances.\footnote{\textcite{Maskelyne1763} explains the advantage of the sextant's 120° range for the lunar distance method. See also Section~\ref{sec:lunar-distance-geometry} of Chapter~\ref{ch:lunar-distance}.}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{generated/ch19-double-reflection}
  \caption{The double-reflection principle. Light from a celestial body reflects off the index mirror to the horizon mirror, where it is combined with the direct view of the horizon. Rotating the mirror by angle $\theta$ rotates the reflected ray by $2\theta$.}
  \label{fig:double-reflection}
\end{figure}

\section{The Octant and the Sextant}

Hadley's original instrument was an \textbf{octant}\index{octant}\index{instruments!octant}---an instrument whose arc spans $90^{\circ}$ (hence the name, from the eight wedges of a circle). An octant can measure angles up to $90^{\circ}$, sufficient for latitude determination (since latitude at Earth's surface ranges from $0^{\circ}$ to $90^{\circ}$). The octant dominated navigation from the 1730s through the 1750s.

But the lunar distance method, as refined by Maskelyne and others, required measuring the angular separation between Moon and Sun (or Moon and star), which could exceed $90^{\circ}$. An octant proved inadequate. The solution was the \textbf{sextant}\index{sextant!design}---an instrument whose arc spans $60^{\circ}$, allowing measurement of angles up to $120^{\circ}$. By Hadley's principle, a $60^{\circ}$ arc with double reflection measures up to $120^{\circ}$ angles. The sextant appeared in the 1750s and quickly became standard.\footnote{\textcite{Cotter1968} traces the evolution from octant to sextant. \textcite{Ifland2005} provides practical context for the instruments' use at sea.}

The advantage of the sextant over a hypothetical $120^{\circ}$-arc single-reflection instrument is not merely compactness. The smaller arc of a sextant is easier to divide precisely. An instrument maker dividing a $60^{\circ}$ arc into 120 parts (for $30'$ divisions) cuts finer than one dividing a $120^{\circ}$ arc into equivalent parts. The geometry favors small, double-reflecting instruments over large, single-reflecting ones.

\section{Optical Components and Construction}

A typical sextant of the 18th and 19th centuries consists of:

\textbf{The frame:} A rigid structure, traditionally of brass or bronze, shaped like a sector with the two radii separated by $60^{\circ}$. The frame carries all other components.

\textbf{The arc:} A brass arc, graduated from $0^{\circ}$ to $60^{\circ}$ (or $0^{\circ}$ to $120^{\circ}$ for double-reading), typically divided into $1^{\circ}$ or $30'$ increments. Each division is hand-engraved or, in later instruments, etched by mechanical dividing engines. The divisions carry significant uncertainty; the skill of the instrument maker determines the accuracy of the entire device.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{generated/ch19-sextant-components}
  \caption{Components of a marine sextant. The index arm rotates about the pivot, carrying the index mirror. Light from a celestial body reflects to the horizon mirror, where it is combined with the direct view of the horizon through the telescope.}
  \label{fig:sextant-components}
\end{figure}

\section{Reading the Angle: Vernier and Drum}

The sextant arc, graduated in $1^{\circ}$ or $30'$ increments, provides a coarse reading. But navigators need precision to the nearest minute of arc ($1'$) or better. Two reading methods evolved:
...
For example, if the index mark is at $47^{\circ}$ on the main scale, and the 6th vernier mark aligns, the reading is $47^{\circ} + 6 \times 0.1' = 47^{\circ} 06'$.
...
To read the drum, the observer notes the degree mark at the main scale, then reads the drum directly. For example, if the main mark shows $42^{\circ}$ and the drum shows $23'$, the angle is $42^{\circ} 23'$.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{generated/ch19-vernier-scale}
  \caption{Reading a vernier scale. The main scale shows whole degrees; the vernier scale allows reading to fractions of a degree by finding where a vernier mark aligns with a main scale mark.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{generated/ch19-altitude-observation}
  \caption{Altitude observation at sea. The observer views the horizon directly through the unsilvered portion of the horizon mirror while the reflected image of the Sun is brought down to touch the horizon line.}
  \label{fig:altitude-observation}
\end{figure}
...
\textbf{Index Error:} The horizon mirror is typically not exactly perpendicular to the plane of the sextant frame. When the index arm points to $0^{\circ}$, the angle between the two mirrors is not exactly $90^{\circ}$ (for an octant) or $120^{\circ}$ (for a sextant). An offset between the two mirror angles creates a constant error in every observation.

To determine index error, the observer points the sextant at the horizon on a clear day. With the index arm at $0^{\circ}$, the observer adjusts the horizon mirror until the direct and reflected images of the horizon appear as a single horizontal line. If the index mark is exactly at $0^{\circ}$, the index error is zero. If the mark is at, say, $+1' 15''$, then the index error is $+1' 15''$. Every observation must be corrected by subtracting this offset.
...
The index mark then reads on the arc. If it reads $0^{\circ} 00' 00''$, the index error is zero. But suppose it reads $0^{\circ} 01' 15''$. This means that when the index arm is positioned to make the horizon appear flat, the index mark has passed the zero point by $1' 15''$.
...
If an observer later measures a star's altitude as $35^{\circ} 22' 30''$, the corrected altitude is

\[
  h_{\text{corrected}} = 35^{\circ} 22' 30'' - 1' 15'' = 35^{\circ} 21' 15''.
\]
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{generated/ch19-sextant-errors}
  \caption{Error sources in sextant observation. Observer reaction time and horizon definition dominate; instrumental errors contribute smaller amounts. The total error (RSS) is approximately 100 arcseconds.}
  \label{fig:sextant-errors}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{generated/ch19-sextant-evolution}
  \caption{Evolution of sextant precision from 1731 to 1900. Improvements in telescope optics, scale graduation, and frame rigidity reduced typical error from $\pm 2'$ to $\pm 15''$.}
  \label{fig:sextant-evolution}
\end{figure}
