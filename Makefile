...
build:
	...
	cp build/tmp/main.pdf build/out/measure-of-the-world.pdf 2>/dev/null || true
	@# Fail the build if common unresolved issues remain:
	@if grep -q "LaTeX Warning: Reference .* undefined" build/tmp/main.log 2>/dev/null; then echo "Undefined references found."; exit 1; fi
